...
Invoke-Command -computername dc -Credential (new-object pscredential("DOMAIN_NETBIOS\PROVISION_USER",(ConvertTo-SecureString -AsPlainText -Force -String "PROVISION_PASSWORD"))) -ScriptBlock {
...
"@
    ...
    {
        Invoke-WebRequest -uri https://wef/api/management/softwareUpdates/gateways/deploymentPackage -UseBasicParsing -OutFile "$env:temp\gatewaysetup.zip" -Credential (new-object pscredential("wef\PROVISION_USER",(convertto-securestring -AsPlainText -Force -String "PROVISION_PASSWORD")))
    ...
    if (-not (Test-Path "C:\Program Files\Microsoft Advanced Threat Analytics"))
    {
        ...
        Start-Process -Wait -FilePath ".\Microsoft ATA Gateway Setup.exe" -ArgumentList "/q NetFrameworkCommandLineArguments=`"/q`" ConsoleAccountName=`"wef\PROVISION_USER`" ConsoleAccountPassword=`"PROVISION_PASSWORD`""
    }
    else
    {

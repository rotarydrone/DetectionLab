...
fix_eth1_static_ip() {
  ...
  echo -e 'interface "eth1" {
    ...
    send dhcp-requested-address LOGGER_IP_ADDRESS;
  }' >> /etc/dhcp/dhclient.conf
  service networking restart
  # Fix eth1 if the IP isn't set correctly
  ETH1_IP=$(ifconfig eth1 | grep 'inet addr' | cut -d ':' -f 2 | cut -d ' ' -f 1)
  if [ "$ETH1_IP" != "LOGGER_IP_ADDRESS" ]; then
    ...
    ETH1_IP=$(ifconfig eth1 | grep 'inet addr' | cut -d ':' -f 2 | cut -d ' ' -f 1)
    if [ "$ETH1_IP" == "LOGGER_IP_ADDRESS" ]; then
      echo "[$(date +%H:%M:%S)]: The static IP has been fixed and set to LOGGER_IP_ADDRESS"
    else
...
install_fleet() {
  ...
  if [ -f "/home/vagrant/kolide-quickstart" ]; then
    ...
    docker run --restart unless-stopped --rm --network=kolidequickstart_default mysql:5.7 mysql -h mysql -u kolide --password=kolide -e 'update app_configs set osquery_enroll_secret = "enrollmentsecret" where id=1;' --batch kolide
    # Set snapshot events to be split into multiple events
    docker run --restart unless-stopped --rm --network=kolidequickstart_default mysql:5.7 mysql -h mysql -u kolide --password=kolide -e 'insert into options (name, type, value) values ("logger_snapshot_event_type", 2, "true");' --batch kolide
...
import_osquery_config_into_fleet() {
  ...
  fleetctl config set --address https://LOGGER_IP_ADDRESS:8412
  fleetctl config set --tls-skip-verify true
  fleetctl setup --email admin@detectionlab.network --username admin --password 'KOLIDE_ADMIN_PASSWORD' --org-name DetectionLab
  fleetctl login --email admin@detectionlab.network --password 'KOLIDE_ADMIN_PASSWORD'

...
post_build_checks() {
  ...
  SPLUNK_CHECK=$(curl -ks -m 2 https://LOGGER_IP_ADDRESS:8000/en-US/account/login?return_to=%2Fen-US%2F | grep -c 'This browser is not supported by Splunk' || echo "")
  FLEET_CHECK=$(curl -ks -m 2 https://LOGGER_IP_ADDRESS:8412 | grep -c 'Kolide Fleet' || echo "")
  ATA_CHECK=$(curl --fail --write-out "%{http_code}" -ks https://WEF_IP_ADDRESS -m 2)
